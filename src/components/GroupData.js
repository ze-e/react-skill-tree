  ...
  return(
    ...
    <ol className="item-data__group">
      {props.children && props.children.length > 0 && props.children.map((child, index) => 
        <ItemData key={index} number={index} item={child} changeName={props.changeName} addLesson={props.addLesson} addChild={props.addChild} addSkill={props.addSkill} deleteItem={props.deleteItem} lastItem={index === props.children.length-1 && true}/>
      )}
    </ol>
    </>
  )
